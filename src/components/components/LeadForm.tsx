...
import { Vertical, MonthlyVolume } from '../../types';
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <select
          ...
          onChange={handleChange}
        >
          {(Object.values(Vertical) as string[]).map((v) => (
        ...
        <div className="md:col-span-2">
          <select
            ...
            onChange={handleChange}
          >
            <option disabled value={MonthlyVolume.INICIO}>
              Volumen mensual de ventas...
            </option>
            {(Object.values(MonthlyVolume) as string[]).map((v) => (
              <option key={v} value={v}>{v}</option>
            ))}
          </select>
        </div>
        ...
        <div className="md:col-span-2">
          <textarea
            name="painPoint"
            rows={4}
            placeholder="Principal dolor o desafío que buscas resolver"
            required
            className="block w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
      ...
      <div className="flex items-start">
        ...
        <div className="ml-3 text-sm">
          <label htmlFor="acceptPolicies" className="font-medium text-gray-700">
      ...
      <div>
        <button
          ...
          className="w-full flex justify-center py-3 px-4 rounded-md text-lg font-semibold text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-indigo-400 disabled:cursor-not-allowed"
        >
